...
exports.createReporting = async (req, res) => {
    const { name, link_template} = req.body;
    try {
      let htmlDesign;
      if (link_template) {
        const existingTemplate = await Reporting.findById(link_template);
        if (!existingTemplate) {
      ...
      const newTemplate = new Reporting({
        name,
        link_template,
        htmlDesign,
      });
      await newTemplate.save();
      res.status(201).json({ message: 'Document template created successfully'});
    } catch (error) {
      res.status(500).json({ message: 'Error creating document template', error });
    }
  };

  exports.getReportingList = async (req, res) => {
    try {
      const page = parseInt(req.query.page) || 1;
      const pageSize = parseInt(req.query.pageSize) || 10;

      const totalReportings = await Reporting.countDocuments();
      const reportingList = await Reporting.find().select('name _id status')
        // .skip((page - 1) * pageSize)
        // .limit(pageSize);
      res.json({
        totalReportings,
        currentPage: page,
        totalPages: Math.ceil(totalReportings / pageSize),
        reportings: reportingList,
      });
    } catch (err) {
      console.error(err);
  ...
  // Get a single reporting document by ID
  ...
  // Update a reporting document
  exports.updateReporting = async (req, res) => {
    try {
      const { htmlDesign, name, htmlContent , documentType, status } = req.body;

      const updatedReporting = await Reporting.findByIdAndUpdate(
        req.params.id,
        { htmlDesign, name, htmlContent, status },
        { new: true }
      );
      if (!updatedReporting) {
        return res.status(404).json({ message: 'Reporting not found' });
      }
      res.json(updatedReporting);
    } catch (err) {
      console.error(err);
      res.status(500).json({ message: 'Server error' });
    }
  ...
  exports.deleteReporting = async (req, res) => {
    try {
      ...
      res.json({ message: 'Reporting deleted successfully' });
    } catch (err) {
      console.error(err);
      res.status(500).json({ message: 'Server error' });
    }
  ...
  // Count total reporting documents
  exports.getReportingCount = async (req, res) => {
    try {
      const totalReportings = await Reporting.countDocuments();
      res.json({ totalReportings });
    } catch (err) {
      console.error(err);
      res.status(500).json({ message: 'Server error' });
    }
