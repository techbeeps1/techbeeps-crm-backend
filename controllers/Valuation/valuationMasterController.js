...
exports.valuationMaster = async (req, res) => {
    try {
        const { customer, package, load, unload, knownAddress, relocation, materials, notes, offer, rooms, jobId, signWithCustomer, sendImmediately } = req.body;
        let job;
        ...
        let finance;
        if (offer?._id) {
            finance = await Finance.findByIdAndUpdate(offer._id, { ...offer,customer: customerExists._id, package: package, Status: signWithCustomer ? 'Accepted' : sendImmediately ? 'Sent' : 'Draft' }, { new: true });
        } else {
            finance = await Finance.create({ ...offer, customer: customerExists._id, package: package, job: job._id, Status: signWithCustomer ? 'Accepted' : sendImmediately ? 'Sent' : 'Draft' });
        }
        ...
        try {
            ...
            return res.status(500).json({ message: 'Error inserting valuation rooms', error: error.message });
        }
